...
{
	return [self initWithDelegate:delegate secret:nil isTest:isTest];
}
...
- (void)validateReceipt:(NSString*)base64EncodedTransactionReceipt
{
	...
	// Construct the post data adding the secret if we have one
	NSString *secretExtra = _secret ? [NSString stringWithFormat:@",\"password\":\"%@\"", _secret] : @"";
	NSString *postData = [NSString stringWithFormat:@"{\"receipt-data\":\"%@\"%@}", base64EncodedTransactionReceipt, secretExtra];
